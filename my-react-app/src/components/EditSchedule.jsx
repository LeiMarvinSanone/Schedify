  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        // Convert 24-hour time to 12-hour format for form
        const [hour24, minute] = schedule.time.split(':');
        ...
        setFormData({
          ...
          alarms: schedule.alarms || [30, 15, 5]
        });
      } catch (error) {
  ...
  const handleConfirmSave = async () => {
    try {
      ...
      const scheduleData = {
        ...
        day: formData.days[0], // For now, take first day
        description: `Class on ${getFullDayName(formData.days[0])}`,
